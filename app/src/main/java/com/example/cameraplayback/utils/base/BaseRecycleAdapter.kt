...
import com.example.cameraplayback.databinding.ItemDefaultBinding
...
abstract class BaseRecycleAdapter<T : Any> : RecyclerView.Adapter<BaseViewHolder<*>>() {
     ...
     *  AddList Not Clear
     */
    open fun addListAll(list: List<T>) {
        _itemList.addAll(list)
        _typeRecycleView = StateRecycleView.STATE_SUCCESS
        notifyDataSetChanged()
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<*> {
        when (viewType) {
            VIEW_TYPE_LOADING -> {
                return setLoadingViewHolder(parent = parent) ?: getDefaultViewHolder(parent = ItemDefaultBinding.inflate(
                    LayoutInflater.from(parent.context)))
            }
            VIEW_TYPE_SUCCESS -> {
                return setNormalViewHolder(parent = parent) ?: getDefaultViewHolder(parent = ItemDefaultBinding.inflate(
                    LayoutInflater.from(parent.context)))
            }
            VIEW_TYPE_EMPTY -> {
                return setEmptyViewHolder(parent = parent) ?: getDefaultViewHolder(parent = ItemDefaultBinding.inflate(
                    LayoutInflater.from(parent.context)))
            }
            VIEW_TYPE_ERROR -> {
                return setErrorViewHolder(parent = parent) ?: getDefaultViewHolder(parent = ItemDefaultBinding.inflate(
                    LayoutInflater.from(parent.context)))
            }
            VIEW_TYPE_DEFAULT -> {
    ...
    /**
     * Thêm danh sách item vào list hiện tại
     * chỉ notify các item vừa được thêm vào danh sách
     */
    fun insertItemToRecyclerview(listDevice: List<T>?) {
        val preSize = _itemList.size
        listDevice?.let { _itemList.addAll(it) }
        notifyItemRangeChanged(preSize, _itemList.size - preSize)
    }
}